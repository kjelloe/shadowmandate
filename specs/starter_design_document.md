# Syndicate Shadows — Sibling Game Design Document

> **SUPERSEDED (2026-07-31):** kept as the historical concept brief. The design of
> record is `01_design_of_record.md`; where they conflict, that document wins
> (notably: hybrid control D1, stealth-first disable-only combat D6 with Snatch
> replacing Assassination, long-lived worlds D3, HQ-extracts-with-you D7).
...
**Relationship:** Sibling game — same engine, same renderer, same art pipeline. Different genre, different feel.

---

## The Pitch

A drop-in / drop-out squad game where every player is an agent working for a Syndicate.
...
The world is always live. Other Syndicates are always operating. The longer you stay, the deeper you go, the more likely you are to cross paths with someone who does not want witnesses.
...
| **Syndicate identity** | You work for a faction with a name, a color, a doctrine. Your HQ flies their flag. |
| **Evac is a mechanic** | Leaving is not a menu option. You call evac, you wait 30 seconds, you extract. |
| **Shared world** | Multiple Syndicates operate simultaneously. Crossing paths is a choice and a risk. |

---

## Relationship to Fireline Command

| Axis | Fireline Command | Syndicate Shadows |
|---|---|---|
| Genre | Tactical RTS / war diorama | Squad action / covert ops |
| Scale | Faction vs faction, full front | 1–4 agents per Syndicate, small ops |
...
- Painted low-poly vehicle chassis (adapted as Syndicate transports)
- Fog of war system
...
A near-future city-region. Corporate towers, industrial zones, port districts, research campuses, and rural outskirts. The Syndicates operate in the gaps between official authority.
...
- **Rival Syndicate presence** (other players' HQs and agents)
...
| Active Syndicates per map | 2–6 |
| Agents per Syndicate | 1–4 |
...
#### Step 1 — Syndicate Briefing (10 seconds, skippable)
...
A low-poly dropship (painted style, Syndicate colors) flies in from the map edge.
...
- A Syndicate flag (faction color).
...
- A **capturable/destroyable site** — rival Syndicates can raid it.
...
| Syndicate Flag | Faction identity, visible on map |
...
The HQ can be raided by rival Syndicates.
...
| **Phase 4 — Cross-District** | 40+ cells | Syndicate War, Territory Seizure | Very high — open conflict |
...
| **Syndicate War** | Open conflict with a rival HQ — raid and destroy | Territory seizure |
...
[TIER 4] Syndicate War — IRON VEIL HQ              ★★★★  open    territory
...
A Syndicate is the player's faction. It has:
...
| **Veil** | Rival Syndicates cannot see your HQ on the map (fog extended) | Counter-intelligence, deep ops |

### Syndicate Names (Curated List)
...
Agents drive Syndicate vehicles — painted low-poly, same chassis system as Fireline Command but smaller and faster.
...
## Multiplayer: Syndicate Squads

### Drop-In Together

Up to 4 agents can operate under one Syndicate flag.
...
**Design note:** "Don't leave anyone behind" is a real decision under pressure. This is the correct emotional beat for a Syndicate squad game.

---

## Crossing Paths: Rival Syndicate Encounters

### How Encounters Happen

Rival Syndicates operate simultaneously on the same map. Encounters happen when:

- Two Syndicates target the same Contract Site.
- A Syndicate raids another's HQ.
- Agents cross paths on a road or transit route.
- A Syndicate's mission requires passing through rival territory.
...
- Both players see each other's Syndicate name and reputation.
...
**Design note:** The standoff is the Syndicate equivalent of the Fireline Command front line. It is the moment of maximum tension and player agency.
...
| **Civilian Traffic** | Move on roads, flee combat | Reveal movement to all Syndicates when fleeing |
| **Neutral Trader** | Crosses map on a fixed route | First Syndicate to escort gets a resource packet |
...
| Syndicate reputation | Per-Syndicate score, decays over time | Low |
...
| Asset | Description |
|---|---|
| Dropship | Low-poly transport, Syndicate-colored livery |
| Field HQ | Command tent + flag + perimeter markers |
| Agent figure | Small humanoid, Syndicate colors, 3 variants per faction |
| Syndicate vehicles | Light transport, armored car, motorbike, cargo van |
...
### Syndicate Color Palettes

| Syndicate type | Primary | Secondary | Accent |
|---|---|---|---|
...
**Recommended intro for Syndicate Shadows:**
...
| Phase | Scope |
|---|---|
| **Alpha** | Single agent, single Syndicate, one district, 3 mission types (Courier, Surveillance, Extraction), dropship in/out, Field HQ, evac beacon |
| **Beta** | Multi-agent squads, 3 Syndicates, rival HQ raids, standoff mechanic, full mission type set |
| **Release** | Full map, 6 Syndicates, all NPC types, Syndicate doctrines, reputation system |
| **Post-release** | Cross-game lore events (Directorate / Outliers appear as Syndicates in Syndicate Shadows) |

---

## The Lore Bridge

Syndicate Shadows and Fireline Command share a world.

The Directorate and The Outliers are not just factions in Fireline Command — they are also Syndicates operating in the same region. A player who has been running Directorate missions in Fireline Command is fighting the same war that Syndicate Shadows agents are operating inside.

This creates a **shared universe** without requiring a shared server:

- Fireline Command: the open front, the war of territory.
- Syndicate Shadows: the covert layer, the war of information and resources.
...
> **In Syndicate Shadows, you work the shadows behind it.**
...
- [ ] HQ is visible to rival Syndicates if fog is cleared.
