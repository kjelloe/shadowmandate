...
| D19 | Tier pacing | Was "median 3–4 deployments to tier 3"; **unit revised by D71** — that assumed the deployment D69d retired. Tier 3 costs 5 completions and a deployment yields 1.19. **Criterion is now TIME, band unset pending Q54.** AND the old number was fiction: `deploysToTier3` counted every Firm's deployments, so 6.0 was ~3x the per-Firm figure — corrected it reads **3.0, in band** |
...
| D69 | The era-2 battery verdicts (Q52) | **Courier 69 -> 110, completing D53** — D53 priced a common 0.115 per WORK-tick and courier has none, so it was never priced at all while the scorer charged it two full legs; it ranked last on every board and the battery read that as a 0.17x preference. Derived from its second leg at D53's own rate; nothing cut, **D42 untouched**. **Partial tier progress persists** (`completedThisTier`) — losing it punished extraction, the core loop, and no world-day battery could see it. **`unlockCompletions` unchanged** pending a clean re-measure. **D11's 40-60 min sortie target retired**: short sorties are correct here |
| D70 | Era-3 reading + follow-through | **Tier progress made VISIBLE** (FIRM tab, both numbers from the engine — the client must not restate the unlock gate). **Sortie band accepted as measured** (13.9-27.8 vs 15-20, overlaps; owner-ruled). **Criterion 10 SPLIT** into 10a sortie (PASS) / 10b tier pace (FAIL, 6.0). **Q53 deferred** to the opposition retune. Era-3: courier **0.17x → 0.59x** and no type flagged over-chosen (era 2 had three); tier-3 pace unchanged — expected, because the battery uses no ledger and so always modelled the no-leak world, meaning D69(b) moved REALITY up to the instrument rather than the number. (The VALUE printed then, 6.0, was separately wrong — D71.) Reach 56% → 64% |
| D71 | Closing the era-3 read | **D19's UNIT revised**: "3-4 deployments" assumed the 40-60 min deployment D69d retired; a deployment now yields 1.19 completions and tier 3 costs 5, so 6.0 is consistent, not a failure. **Time is the criterion**, `ticksToTier3` added to measure it, **band unset pending Q54**; `unlockCompletions` untouched. **Dawn wait** left as a playtest question. **Acquisition 0.34x is unreadable** — tier gating + 2.6% board share + 27% declined on credentials; do not tune against it. **D69(b) accepted as proven by argument** — no battery crosses a ledger |
| D72 | Corrected tier-3 measurement | `0005-pacing` n=300: **deploys to tier 3 = 2.0, LOW against 3–4** — progression was never slow, it is faster than the band. 0004 vs 0005 differ in **zero cells across 46 shared columns**; the 6.0/2.0 ratio is exactly 3.0, the AI Firm count. Time to tier 3 **47 min AI / 94–188 human, 64% arrive**. **A second scope error, mine**: D71(a)'s "1.19 completions per deployment" is world-wide while D11's 2–3 is per Firm — the reaching Firm does **2.5, inside intent**. No verdict drawn from 2.0: it is in the unit D71 revised away from. Band still Q54; `unlockCompletions` untouched |
| D73 | The 36% diagnosed; scope declared | **`beamTripped` 1687 vs 0** — read as the heat spiral, and **that was wrong (see D74)**: a beam is LEVEL-triggered (fires every tick an agent STANDS in it) while it implements a CROSSING — 614 consecutive ticks measured. Waiting is the game working (D41 patrol window, WD-1 dark), and nobody asserted the stationary case. **Diagnosed, NOT fixed → Q55.** Q54's band stays unset and **the deploy is HELD** until the 36% resolves. Every battery column now declares its scope, guard-enforced; `heatLockdownTicks` → **`districtLockdownTicks`** |
| D74 | Beam edge-triggered; D73 corrected | Beam fires on ENTRY, not every tick (`beam.inside` = inside AND LIVE, so a beam coming back on still trips a camper). **`beamTripped` 1687 → 36.** **But it fixed nothing else — D73's causal claim was WRONG**: `raiseAlarm` is idempotent while an alarm is live, so outcomes are byte-identical. Real drivers by reason: `sustained` 144 vs 18, `burned` 97 vs 25, `beam` **8 vs 0**. Kept because correct, not because it solved the spiral. Era `sm-era-4` |
