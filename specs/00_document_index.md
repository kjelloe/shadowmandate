...
| D73 | The 36% diagnosed; scope declared | **`beamTripped` 1687 vs 0** is the heat spiral: a beam is LEVEL-triggered (fires every tick an agent STANDS in it) while it implements a CROSSING — 614 consecutive ticks measured. Waiting is the game working (D41 patrol window, WD-1 dark), and nobody asserted the stationary case. **Diagnosed, NOT fixed → Q55.** Q54's band stays unset and **the deploy is HELD** until the 36% resolves. Every battery column now declares its scope, guard-enforced; `heatLockdownTicks` → **`districtLockdownTicks`** |
