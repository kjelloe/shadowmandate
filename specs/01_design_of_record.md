...
| **D19** | **Tier pacing.** Originally "median 3–4 deployments to reach tier 3". **REVISED BY D71**: that unit assumed the 40–60 min, 2–3 contract deployment D69d retired. A deployment yields **1.19 completions** WORLD-WIDE — but that average includes every Firm, and D11's "2-3 contracts per deployment" is per Firm. The Firm that actually reaches tier 3 does **2.5** (5 completions across a measured 2.0 deployments), which is **inside D11's original intent**; the 1.19 is dragged down by deployments that complete nothing. **And the 6.0 it was failing against was a broken measurement** — `deploysToTier3` counted every Firm's deployments, not the reaching Firm's; corrected it reads **3.0, inside the original band**. **The criterion is TIME to tier 3** (`ticksToTier3`); the deployment count is kept as a diagnostic. **The band is UNSET pending Q54** — the ambition (tier 3 is a mid-term goal, not a grind) is unchanged; only the ruler is. |
...
| # | Ruling |
|---:|---|
| **D63** | **Three rulings in one playtest.** (a) **Night sneak: 30%** — at night, watchers (patrols, cameras) see 30% shorter; the agent's own sight is untouched. The day-night build (S03/S09 drafts) is unblocked. (b) **World scale, final ratios**: the 1/16 figures were a few pixels; the ruled read is figures 8x taller on screen with roads 4x wider — decomposed as figures at **1/8 of a cell** (2x relative) plus a much closer camera (default 3.5 cells across, minimum 1.5, where the full 8x lands); street furniture re-proportioned to match. (c) **MISSION AREAS — the revision of D45.** Waiting out a work timer is not gameplay. Extraction and surveillance gain a playable INSIDE: "Begin" at the site enters a guarded 3D area — sneak, bloodless takedowns (D6 holds), terminals that kill alarms — running on the SAME world tick as everything else, with other players entering the same area to help or sabotage. D45's modal-minigame objection dies because the area is a PLACE in the shared world, not a paused screen; the multi-team Firm-HQ assault is the long goal. Spec S17; extraction and surveillance areas build together, shared-capable by construction. Plus the city-life directive: patrols 4/district now; civilians (mind their business, flee trouble) and hover cars specced as engine-side ambient life. |
| **D64** | **Mission-area shape (2026-08-24).** Compound scale ~24x16 with courtyards; **burned inside = play on** under escalating lockdown (D44's worsening-situation doctrine, indoors); **PvP takedowns inside = yes** — a rival can put you down and leave you for the guards, that IS sabotage. Ambient city-life numbers delegated (built per S17 proposals: civilians 8/district engine-side, hover cars client theatre). *(Recorded in the index at the time; this row was missed — added in the 2026-08-27 sweep.)* |
| **D65** | **2026-08-27, four rulings in one directive.** (a) **Era discipline enforced in metal** (Q47-A): version `sm-era-1`; pre-bump sweeps mechanically void; the version is hashed in both twins — the documented contract said so since M0 and the bytes never existed. (b) **The AI is a customer** (4A): credentials bought from the HQ cache with player commands (payBail's funding split extended to dialogue/shop, isAi-scoped so D30 still binds players); the scorer nets the badge price; entry to mission areas defers to nightfall when at most `waitForNightTicks` away. (c) **The cyberpunk grade** (ref `debugging/cyperpunk-example.png`): street-level splash (perspective camera, parallax haze skylines, edge neon) and district round two as facade neon + industrial pipes — palettes/densities in tokens, like every other look judgement. (d) **Waiting polish**: a locked-down safehouse still shelters (only the informant's wares go quiet), and the wait shows an engine-computed nightfall countdown. Avenue density noted for later (7B). |
| **D66** | **Playtest 13 (2026-08-28), eight findings.** (a) **The camera moves**: right-drag pans (stored as an OFFSET from whatever is followed, so the view still travels with the operative) and rotation in FOUR quarter-turns off 45 degrees — the standing "no player rotation" rule is relaxed, because a fixed compass makes the two rear facades permanently unknowable and a stealth game must not hide the board. (b) **The client draws on rAF**, not per snapshot: the reported "jerky, lag skip" was ten hard jumps a second. Movers ease and SNAP past 4 cells; every per-frame slew is dt-based, because easing speed must not be a property of the player's monitor. (c) **Figures 41% taller, 79% leaner** (1.03->1.455; ratio 1.78->3.18) for the trench silhouette — the splayed ARMS, not the coat, were what read stocky. (d) **Cover shops are standing landmarks** in world and radar, not a burned-only ping; **alerted patrols scale and pulse** rather than only changing hue. (e) **A captured Firm is told its options** — bail and the D51 fold both existed for months and were never on screen. (f) **Mission areas gain four floor plans** derived from site type, and the compound camera follows at street zoom, which IS the ruled 8x (the old whole-compound fit put 28 cells across a 3.5-cell frame). (g) **Districts gain smoke and parks.** (h) **Two typographic voices**: display face for chrome, plain monospace ink for anything read to decide. |
| **D67** | **2026-08-28/29, three owner rulings.** (a) **Q50 — the picker stops lying**: safehouses 1 -> **8 per district** and `hqLandingFor` is confined to the district of the requested cell. 62% of drops used to land somewhere the player had not chosen; it is 0% now, exact rather than statistical (no distance bound got below ~4%, because a border drop legitimately has a nearer safehouse across the line). At the old density, 51% of drops with four rivals deployed got NO building at all. Worldgen change: **era `sm-era-2`**, fixtures re-pinned, era-1 baselines void at zero cost because those batteries had not run. A third change follows: an unclear door is now the LAST resort, behind the tent on certified-clear ground — playtest 5 ruled out being burned before you have control. (b) **Q48 — the mid-sortie redrop** (`CMD_REDROP`): `bail.redropReputationHit` finally reads to something. Folding EXTRACTS and banks the cache; redropping keeps you earning with the cache at risk and costs standing. The AI got it in the same slice. (c) **Q49 — patrol sight at CALM heat**: owner playtesting, nothing changed. |
| **D68** | **City Info (2026-08-28/30, Q51).** One tabbed panel on the splash and in the field: board, log, kit, firm, sortie, history, city, places, districts, firms, legend. **Rival intel is EARNED OR BOUGHT ONLY** — the roster is lawful (a Firm operating in a city is not a secret, and the informant has always sold HQ locations), but a position crosses the wire only when seen or bought, gated at the VIEW because a client cannot un-leak a field it was given. **The human/AI split is never disclosed** and `isAi` is not sent at all. **Standing is "yours, not theirs"** — a position and a count, no rival reputation. The BOARD folds in as the default tab, its click-safety preserved by showing/hiding panes rather than rebuilding them. The LEGEND is derived from `tokens.marks`, so a new mark cannot go undocumented. Only HISTORY needed persisted state (`sorties`, `bankedTotal`, `completedByKind`, normalised on read so no file migrates). **KIT was not asked for and mattered most**: `credentialTier` was not in the view at all, so a badge bought from the vendor was invisible while the 8f rule gated the work behind it. |
| **D69** | **The era-2 battery verdicts (2026-08-30, Q52).** Four rulings, and the first two are the same lesson twice: **a number nobody computed reads exactly like a number badly chosen.** (a) **Courier's reward 69 -> 110, COMPLETING D53 rather than revisiting it.** D53 levelled pay-per-effort at a common **0.115 per WORK-tick** — and courier has ZERO work ticks, so the pass computed nothing for it and its 69 stood as a pre-effort-pricing leftover. Meanwhile the AI scorer charges `1 + distance + workCells` where distance is Manhattan HQ->site **plus the second leg**, and courier is one of only three two-site types (median total 62-63 cells against 29-31 for one-leg work). So courier paid the longest journey in the game for the smallest purse and ranked **dead last on every board the AI ever saw** — which the battery reported as a 0.17x preference, i.e. as a balance problem. 110 is DERIVED, not chosen: the ~34-cell second leg above the one-leg baseline every type walks, at 28 ticks/cell, at D53's own rate. It lands courier mid-pack (score ~440, beside extraction's 429), never top. **Nothing was cut, so D42 stands untouched.** (b) **Partial tier progress now survives extraction** (`completedThisTier` persisted). `tierUnlocked` was kept and progress toward it was discarded, so a Firm four contracts into a five-contract tier lost all four **by going home** — in a drop-in/drop-out game, a penalty on the core loop. **No battery could ever have seen it**: a world-day runs continuously and never extracts, so the tier-3 figure printed at the time was OPTIMISTIC for players. (That figure, 6 deploys, was ALSO wrong for an unrelated reason — D71 found it counted every Firm's deployments; the real per-Firm number is 3.0, in band.) It is the one field here that legitimately goes DOWN (a tier crossing resets it), so it assigns where its neighbours max. (c) **`unlockCompletions` is NOT changed.** The leak may be the cause; changing the gate in the same era would make the next verdict unreadable. Re-measure first. (d) **D11's 40-60 minute sortie target is RETIRED.** The batteries show more, shorter sorties and that is correct for a drop-in/drop-out game — the figure predates the drop-in loop. No future battery is to be read against it. |
| **D70** | **The era-3 reading and four follow-through rulings (2026-08-30).** (a) **Tier progress is now VISIBLE.** `completedThisTier` persists (D69b), which made it a durable thing the player owns — and it was in no view at all, so a kept asset looked exactly like a lost one. The FIRM tab reads "N of M contracts toward tier T". **Both numbers cross the wire**: the client must never recompute M from `unlockCompletions`, because that is the contract machine's gate and a second reader of a rule is how 8f ended up known to the contracts and not to the AI. Three honest states — a fraction, "top tier reached" (M is 0 there, and "0 of 0" is a lie rather than a number), and unknown before a view exists. (b) **The sortie band stands as measured.** Era-3 gives 13.9-27.8 min human-adjusted against D11's 15-20; the owner accepted that range. It OVERLAPS rather than sits inside, and that is the ruling, not a deferral. (c) **Acceptance criterion 10 is SPLIT** into 10a (sortie length, D11 — PASS) and 10b (3-4 deploys to tier 3, D19 — FAIL at 6.0). Two rulings had been bundled into one criterion, so each was dragging the other into a permanent PARTIAL. (d) **Q53 deferred deliberately**: acquisition and sabotage walk the same unpriced second leg courier did (~+107 each at D53's rate), and pricing them now would make them the top two contracts on an untested rate. They are exactly the types D42 says get harder as a season progresses, and difficulty changes what they are worth — pricing them before the opposition retune means pricing them twice. **THE ERA-3 VERDICT ITSELF**: courier 0.17x -> **0.59x**, and the spread collapsed — era 2 flagged three types over-chosen (surveillance 1.62x, sabotage 2.35x, defend 2.03x) and era 3 flags **none**. Tier-3 pace is unchanged at 6.0 — later found (D71) to be an inflated metric counting every Firm; the real per-Firm figure is 3.0. The *unchanged* part was still the expected result, and the important subtlety: the world-day harness uses **no ledger**, and `extractHq` never cleared `completedThisTier`, so the battery was ALWAYS modelling the no-leak world. D69(b) did not move the measurement — **it moved reality up to meet it**, and the figure is now a trustworthy player one where before it was optimistic (though the VALUE printed at the time, 6.0, was separately wrong — see D71). Reach rate 56% -> 64%. The gate (`unlockCompletions`) is now decidable on honest data. |
| **D71** | **Four rulings closing the era-3 read (2026-08-30).** (a) **D19's UNIT is revised, not its ambition.** "Median 3-4 deployments to tier 3" was written when a deployment meant 40-60 minutes and 2-3 contracts — **the same assumption D69d retired for D11**, and retiring one while grading against the other measures in a unit whose meaning changed. **And the number it was failing against was never real.** `deploysToTier3` read `m.deployments` — EVERY Firm's deployments — at the moment the FIRST Firm reached tier 3, so with 3 AI Firms it reported roughly 3x the per-Firm quantity D19 asks about. Corrected to count only the reaching Firm, the same worlds read **3.0: INSIDE the 3-4 band**. Every tier-3 verdict this project has printed was that inflated figure (5.0 at M8, 6.0 on eras 2 and 3, all graded HIGH). **D19 was never failing, `unlockCompletions` never needed changing, and the "progression is slow" reading from Q52 onward was an instrument defect.** The count is retained as a diagnostic; **TIME is the criterion now**, and `ticksToTier3` was added to the world-day instrument to measure it — the tick was already tracked to compute `deploysToTier3` and simply never emitted. **No band is set here**: the owner sets it once measured (Q54), because printing a target nobody ruled is exactly how a retired one keeps manufacturing verdicts. `unlockCompletions` remains untouched. (b) **The dawn wait is a playtest question, left open.** An 18-min day phase against a 24.8-min short-end deployment means a dawn drop waits most of a sortie for dark. Whether that is tension or dead time is a judgement no battery makes; flagged in `data/season.json` so it cannot be forgotten. (c) **Acquisition's 0.34x is NOT READABLE as a preference** and must not be tuned against. It is three suppressors compounding: tier-3 gated (only 64% of Firms ever arrive), 2.6% of the board, and **27% of its pool declined outright** for want of an affordable credential (8f). Extraction loses 33% the same way and absorbs it only because it is abundant. Q53's deferral stands. (d) **D69(b) is accepted as proven by argument.** No world-day battery crosses a ledger, so none can confirm it; the reasoning and the round-trip unit test stand in place of a measurement, recorded in S14 rather than left implicit. |
| **D72** | **The corrected tier-3 measurement (2026-08-30, `0005-pacing`, n=300, era 3).** The first battery to measure what D19 actually asks about, and it inverts a belief this project held for months. **Deployments to tier 3: 2.0**, against a band of 3–4 — **LOW, not HIGH**. Progression was never slow; it is, if anything, faster than the band intended. The old 6.0 and the new 2.0 come from **byte-identical worlds**: 0004 and 0005 differ in **zero cells across 46 shared columns and 300 rows**, and the same 193 Firms reach tier 3 in both. The ratio 6.0/2.0 is exactly **3.0 — the AI Firm count**, which is the defect's signature. That is also a free determinism confirmation across two commits. **Time to tier 3: 47.0 min AI, 94–188 min human-adjusted, 64% of Firms arriving.** **A second scope error, mine, inside D71 itself**: D71(a) argued from "a deployment yields 1.19 completions against D11's 2–3 intent" — but 1.19 is a WORLD-WIDE average over all Firms while D11's intent is per Firm. The Firm that reaches tier 3 does **2.5 completions per deployment** (5 completions across 2.0 deployments), which is **inside D11's original intent**. The 1.19 is depressed by deployments that complete nothing. Corrected in D19's row. **The category error was committed twice — once in the instrument, once in the ruling that diagnosed the instrument.** **No verdict is drawn from the 2.0.** "LOW against 3–4" is a reading in precisely the unit D71(a) revised away from, because "deployment" changed meaning when drop-in/drop-out made them short and frequent. The time figure is the criterion and **still has no band** — that is Q54. `unlockCompletions` remains untouched, now for the strongest reason yet: nothing measured says it is wrong. |
| **D73** | **Four rulings, and the diagnosis they produced (2026-08-31).** (a) **Diagnose before acting** on the 36% of worlds that never reach tier 3 — and it found a real engine defect. Those worlds are not slow, they are **locked down**, and the event census names the cause without ambiguity: **`beamTripped` 1687 against 0** between spiral and healthy worlds, with `alarmEscalated` 8.0x, `contractCompleted` 0.26x and `tierUnlocked` 0.11x alongside it. **CORRECTED BY D74: that was a CORRELATION, not the cause.** Fixing the beam cut `beamTripped` to 36 and changed **nothing else** — `raiseAlarm` is idempotent while an alarm is already live, so the repeat trips never drove the escalation. The real drivers are `sustained` escalation (144 vs 18) and `burned` raises (97 vs 25); beam-reason alarms are **8 vs 0**. A beam is **LEVEL-triggered** — it fires for an agent *standing in* it, once per beam per tick — while the design it implements is a **crossing**, as its own comment says. An agent that stops becomes a permanent siren: **614 consecutive ticks** measured on one beam. Stopping is the game working (D41's patrol window makes waiting the decision; WD-1 waits for dark), and nothing ever asserted what a beam does to a stationary agent. **The doctrine's own "every mechanism must have a usable gap" was asserted for a CROSSING and never for a wait** — a mechanism verified against its intended use can still be pathological in a state the verification never modelled. Deliberately **NOT fixed**: the remedy is a design choice → **Q55**. Two earlier hypotheses (credential gating, then civilians tripping beams) were both wrong and discarded on evidence, which is why the census was taken whole rather than as a hand-picked list. Probe: `debugging/dbg_heat_spiral.mjs`. (b) **Q54's band stays unset** until the 36% resolves: time-to-tier-3 is measured only among Firms that arrive, so fixing the spiral moves both who arrives and how long they take, and any band set now is fitted to a distribution about to change. (c) **The deploy is HELD** for the same reason. A third of cities becoming unwinnable is what a first player meets and does not return from; the runbook, unit, port and index entry are ready and keep. (d) **Every battery column declares its scope** (`world` / `firm` / `district` / `config`), guard-enforced, after three wrong readings in one day from world-scope numbers whose names read as per-Firm. `heatLockdownTicks` is renamed **`districtLockdownTicks`** — it accumulates per district, so its ceiling is ticks x districts and it was nearly read as a fraction of the run. This does not prevent a scope error; it prevents an UNDECLARED one, which is the version nobody can catch by reading the CSV. |
| **D74** | **The beam is edge-triggered now — and it did NOT fix the 36% (2026-08-31, Q55, era `sm-era-4`).** Owner ruled edge-trigger. Shipped: a beam fires when an agent ENTERS it and not again until they leave. `beam.inside` records who was tripping it last tick, meaning **inside AND LIVE** rather than merely inside — an agent who waits out the dark window and is still standing there when the beam returns is still tripped, or "wait for the gap" degenerates into "stand in the gap forever". Both directions are asserted and mutation-tested. Measured: **`beamTripped` 1687 → 36**, a 47x cut in wire and client noise. **AND IT CHANGED NOTHING ELSE. The D73 diagnosis was wrong about causation.** Every outcome on the same seeds is byte-identical: `alarmEscalated` still 144, completions still 0/2/1/3, the spiral untouched. **`raiseAlarm` is idempotent while an alarm is already live** and `setStage` only emits on a stage CHANGE, so 1687 trips and 36 trips produce the same alarm state. I read the most dramatic column in an event census as the cause; it was a symptom of an agent parked in a beam, and the parking is downstream of something else. **Where the spiral actually comes from**, by alarm REASON: `sustained` escalation **144 vs 18**, `burned` raises **97 vs 25**, `camera` 3 vs 1, `beam` **8 vs 0**. Burns are only 1.25x more common in spiral worlds but produce 3.9x the alarms, and `alarmEased` runs 119 vs 15 — so alarms oscillate up and down rather than clearing. The working hypothesis is a burned agent LINGERING near a site (D41 makes waiting for the patrol window the correct play) holding an alarm in sustained escalation. **Not investigated further and not fixed.** **The change is kept because it is correct on its own terms** — the code's own comment always said "crossed", and 1687 events where 36 suffice is client and wire noise — **not because it solved anything**. Era bumped: new hashed state in both twins, deep-copied in `copyState` and `mirrorState`, because a spread would share the array by reference (the S17 trap). |
