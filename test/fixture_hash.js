// test/fixture_hash.js — the DELIBERATE DUPLICATE of engine/snapshot.js
// hashState. Both copies must change together; that is the entire point.
//
// If you are here because a hash test failed: do not "fix" this file to match
// the engine. Ask first whether the engine change was intended to alter the
// state hash. If it was, change BOTH copies and re-pin the fixture with a
// written reason. If it was not, the engine is wrong.

import { createByteWriter, computeFnv1a64, hashToHex64 } from "../shared/canonical.js";

export function hashStateLocal(state) {
  ...
  for (const f of state.firms) {
    if (f.state === 0 && f.reputation === 0 && f.recognition === 0
        && f.cacheResources === 0 && f.isAi === 0) continue;
  ...
  for (const x of (state.beams ?? [])) {
    ...
    // D74: the deliberate duplicate of snapshot.js's beam occupant writer.
